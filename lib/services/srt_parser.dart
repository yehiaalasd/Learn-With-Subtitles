...
class Range {
  const Range(this.begin, this.end, this.beginDuration, this.endDuration);
...
@visibleForTesting
void parseHtml(ExtractedSubtitle subtitle) {
...
@visibleForTesting
void parseCoordinates(ExtractedSubtitle subtitle, String chunk1) {
...
Range? parseBeginEnd(String line) {
  ...
  if (match == null) {
    ...
    final Duration be = timeStampToMillisDuration(
        int.parse(match.group(1)!),
        int.parse(match.group(2)!),
        int.parse(match.group(3)!),
        int.parse(match.group(4)!));

    final Duration en = timeStampToMillisDuration(
        int.parse(match.group(5)!),
        int.parse(match.group(6)!),
        int.parse(match.group(7)!),
        int.parse(match.group(8)!));
    return Range(begin, end, be, en);
  }
}

Duration timeStampToMillisDuration(int hour, int minute, int sec, int ms) {
  Duration duratio =
      Duration(hours: hour, minutes: minute, seconds: sec, microseconds: ms);
  return duratio;
}
...
List<ExtractedSubtitle> parseSrt(String srt) {
  final List<ExtractedSubtitle> result = [];
  ...
  for (List<String> chunk in splitChunk) {
    ...
    final ExtractedSubtitle subtitle = ExtractedSubtitle(
